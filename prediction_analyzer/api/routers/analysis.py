...
async def get_global_analysis(
    ...
    summary = analysis_service.get_global_summary(db, user_id=current_user.id, filters=filters)
...
async def get_market_analysis(
    ...
    exists = (
        db.query(Trade)
        .filter(Trade.user_id == current_user.id, Trade.market_slug == market_slug)
        .first()
    )
...
async def get_advanced_metrics(
    ...
    """
    Calculate advanced trading metrics: Sharpe ratio, Sortino ratio,
    ...
    trades = analysis_service.get_filtered_trades(db, user_id=current_user.id, filters=filters)
...
async def get_market_breakdown(
    ...
    """
    breakdown = analysis_service.get_market_breakdown(db, user_id=current_user.id, filters=filters)
...
async def list_saved_analyses(
    ...
    return [SavedAnalysisResponse(**analysis_service.parse_saved_analysis(a)) for a in analyses]
...
async def save_analysis(
    ...
    """
    saved = analysis_service.save_analysis(db, user_id=current_user.id, analysis_data=analysis_data)
...
async def get_saved_analysis(
    analysis_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    if not analysis:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Saved analysis not found"
        )
