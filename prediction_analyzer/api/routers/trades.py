...
async def list_trades(
    ...
    source: Optional[str] = Query(
        None, description="Filter by provider: limitless, polymarket, kalshi, manifold"
    ),
...
async def upload_trades(
    ...
    try:
        upload_id, trade_count = await trade_service.process_upload(db, current_user.id, file)
...
async def export_trades_json(
    ...
    """
    trades, _ = trade_service.get_user_trades(
        db, user_id=current_user.id, limit=100000, offset=0, market_slug=market_slug
    )
...
async def get_trade(
    trade_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
...
async def delete_all_trades(
    ...
    return {"message": f"Deleted {count} trades", "deleted_count": count}
